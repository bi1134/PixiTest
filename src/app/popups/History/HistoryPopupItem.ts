...
import { HistoryDetailApiResponse, mockHistoryDetailResponse } from "../../../api/models/HistoryDetailResponse";
import { formatFloatNumber, formatIntNumber } from "../../../utils/format-number";
...
export class HisotryPopupItem extends Container {
    ...
    constructor() {
        ...
        this.betAmount.position.set(
            this.OFFSET,
            this.bgItem.height * 0.3
        );
        ...
        this.expandSwitcher.onChange.connect(this.onExpandSwitcherChange.bind(this));
        ...
        this.textWrapper.addChild(
            this.idText,
            this.betId,
            this.copyIdButton
        );
        this.textWrapper.pivot.set(this.textWrapper.width / 2, 0);
        this.textWrapper.position.set(
            this.bgItem.width / 2,
            this.OFFSET
        );
    ...
    public setHistoryDetailData(response: HistoryResponseData) {
        ...
        if (Number.isInteger(betAmount))
            ...
            this.betAmount.text = `Taruhan: ${formatFloatNumber(betAmount)}`;
        ...
        const profitNumber = response.multiplier === 0 ? response.amount : response.total_win;
        if (Number.isInteger(profitNumber))
            this.profitText.text = `${sign}Rp ${formatIntNumber(profitNumber)}`;
        else
            this.profitText.text = `${sign}Rp ${formatFloatNumber(profitNumber)}`;

        // Check status to fill color
        if (response.multiplier !== 0)
            this.profitText.style.fill = '#5FFF44';
        else
            this.profitText.style.fill = '#FFDE45';
        ...
        if (Number.isInteger(muliplier))
            ...
            this.multiplier.text = `Mult.${formatFloatNumber(muliplier)}x`;
        ...
        this.board.children.forEach((value, index) => {
            ...
            if (field.includes(index)) {
                if (bomb_field.includes(index))
                    ...
                    sprite.texture = Sprite.from("crown_selected.png").texture;
                ...
                if (bomb_field.includes(index))
                    ...
                    sprite.texture = Sprite.from("crown_not_selected.png").texture;
    ...
    private onExpandSwitcherChange(state: number | boolean) {
        ...
        if (state) {
            ...
            gameService.postHistoryDetail(this.betId.text).then((response: HistoryDetailApiResponse) => {
