...
export class ProfitLayout extends Container {
  ...
  private createLayout() {
    this.profitBackground = new Graphics()
      .rect(0, 0, 300, 150)
      .fill("#3c3c3cff");
  ...
  public resize(width: number, height: number, padding: number) {
    ...
    LayoutHelper.setPositionX(
      this.pLowerItem,
      this.pHigherItem.x + itemWidth + padding,
    );
    LayoutHelper.setPositionX(
      this.pTotalItem,
      this.pLowerItem.x + itemWidth + padding,
    );
    ...
    const scaledBgH =
      this.pHigherItem.getBackgroundHeight() * this.pHigherItem.scale.y;
