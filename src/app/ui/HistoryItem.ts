...
export class HistoryItem extends CustomDropdownBox {
    constructor(title: string, data: any) {
        ...
        const arrow = new Text({ text: "▼", style: { fill: 0x999999, fontSize: 14 } });
        ...
        const addRow = (key: string, value: string) => {
            ...
            const keyText = new Label({ text: key, style: { fill: 0xaaaaaa, fontSize: 16 } });
            keyText.x = 20;

            const valText = new Label({ text: value, style: { fill: 0xffffff, fontSize: 16, fontWeight: "bold" } });
        ...
        const line = new Graphics().moveTo(10, yPos + 5).lineTo(bg.width - 10, yPos + 5).stroke({ width: 1, color: 0x444444 });
        ...
        const btnLabel = new Label({ text: "Replay", style: { fill: 0xffffff, fontSize: 14 } });
        ...
        const spacer = new Graphics().rect(0, 0, 1, btnContainer.y + btnContainer.height + 15).fill({ alpha: 0 });
