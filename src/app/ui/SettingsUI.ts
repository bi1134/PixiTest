...
export class SettingsUI extends Container {
    ...
    public resize(width: number, height: number) {
        ...
        LayoutHelper.setPositionY(this.bgSetting, this.settingsSwitcher.y - this.bgSetting.height / 2 - padding);
        ...
        LayoutHelper.setPositionY(this.historyIcon, this.audio.y - this.audio.height - padding);
        LayoutHelper.setPositionY(this.questionMark, this.audio.y + this.audio.height + padding);

        LayoutHelper.setPositionX(this.settingText, this.settingsSwitcher.x + this.settingsSwitcher.width / 2);
        LayoutHelper.setPositionY(this.settingText, this.settingsSwitcher.y + this.settingsSwitcher.height + padding);
    }
    ...
    private updateVisibleUI(state: number | boolean) {
        if (state) {
            gsap.to(this.bgSetting, {
                ...
                ease: 'back.out',
                scale: 1,
                onStart: () => { this.bgSetting.visible = true; }
            });
            ...
            gsap.to(this.audio, {
                ...
                ease: 'back.out',
                scale: 1,
                onStart: () => { this.audio.visible = true; }
            });
            gsap.to(this.historyIcon, {
                duration: 0.1,
                ease: 'back.out',
                scale: 1,
                onStart: () => { this.historyIcon.visible = true; }
            });
            gsap.to(this.questionMark, {
                duration: 0.1,
                ease: 'back.out',
                scale: 1,
                onStart: () => { this.questionMark.visible = true; }
            });
        }
        else {
            gsap.to(this.bgSetting, {
                ...
                ease: 'back.in',
                scale: 0.75,
                onStart: () => { this.bgSetting.visible = false; }
            });
            ...
            gsap.to(this.audio, {
                ...
                ease: 'back.in',
                scale: 0.75,
                onStart: () => { this.audio.visible = false; }
            });
            gsap.to(this.historyIcon, {
                duration: 0.1,
                ease: 'back.in',
                scale: 0.75,
                onStart: () => { this.historyIcon.visible = false; }
            });
            gsap.to(this.questionMark, {
                duration: 0.1,
                ease: 'back.in',
                scale: 0.75,
                onStart: () => { this.questionMark.visible = false; }
            });
        }
